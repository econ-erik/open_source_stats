...
ui_text <- pdf_text(paste(data_path,"ui_claims_may09.pdf",sep="")) # fill in most recent pdf
...
ui_claims_1 <- ui_claims_1 %>%
  ...
  arrange(abbr)
# write to csv
write_csv(ui_claims_1,paste(data_path,"ui_claims_",today(),".csv",sep=""), col_names = TRUE)
...
ui_claims_2 <- ui_claims_2 %>%
  ...
  arrange(abbr)
# write to csv
write_csv(ui_claims_2,paste(data_path,"ui_continue_",today(),".csv",sep=""), col_names = TRUE)
